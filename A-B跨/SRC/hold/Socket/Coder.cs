...
namespace hStore.GPRS
{
    ...
    public class Coder
    {
        ...
        {
            byte[] rbyte = new byte[Encoding.UTF8.GetBytes(datagram).Length + 2];
            rbyte[0] = 0x23;
            rbyte[1] = 0x23;
            switch (_encodingMothord)
            {
                case EncodingMothord.Default:
                    {
                        Encoding.Default.GetBytes(datagram, 0, datagram.Length, rbyte, 2);
                        return rbyte;
                    }
                case EncodingMothord.Unicode:
                    {
                        Encoding.Unicode.GetBytes(datagram, 0, datagram.Length, rbyte, 2);
                        return rbyte;
                    }
                case EncodingMothord.UTF8:
                    {
                        Encoding.UTF8.GetBytes(datagram, 0, datagram.Length, rbyte, 2);
                        return rbyte;
                    }
                case EncodingMothord.ASCII:
                    {
                        Encoding.ASCII.GetBytes(datagram, 0, datagram.Length, rbyte, 2);
                        return rbyte;
                    }
                default:
                    {
